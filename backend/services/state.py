...
def news_items():
 return [{"source":"Yahoo/RSS","ticker":"AMD","title":"AI semis strength continues","impact":"Positive","action":"Watch, do not chase extended move"},{"source":"Macro","ticker":"SOFI","title":"Yields and credit sentiment in focus","impact":"Mixed","action":"Rates sensitive"},{"source":"Advisor Intel","ticker":"NBIS","title":"Chart thread: resistance breakout watch","impact":"High","action":"Day trade only due portfolio weight"}]

def catalyst_calendar():
 base=datetime.now().date()
 return [{"date":str(base+timedelta(days=2)),"event":"CPI","impact":"Macro volatility"},{"date":str(base+timedelta(days=5)),"event":"NVDA earnings","impact":"AI basket read-through"}]
